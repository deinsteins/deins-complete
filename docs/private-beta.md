...
This private beta sends bounded cursor-adjacent code context to the DeinsComplete backend, which may forward it to configured AI providers. Repository Context is enabled by default and may add small snippets from relevant local imports and open workspace files, compact VS Code definition/signature signals, relevant package names from `package.json`, and categorical formatting preferences; it never uploads lockfiles or indexes the entire repository. Dependency declaration files may be read locally for bounded symbol signatures but their full content is not sent. Disable it per workspace with `deinscomplete.repositoryContext.enabled`. Do not use it with code you are not authorized to send for AI processing.
