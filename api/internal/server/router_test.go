...
func testRouter() http.Handler {
	...
	return newRouter(logger, completion.NewService(providers.MockProvider{}), auth.New("", 1, 0), false, false, nil, nil, nil)
}
...
func TestProviderErrorsUseSafeAPIResponses(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	router := newRouter(logger, completion.NewService(providerErrorProvider{}), auth.New("", 1, 0), false, false, nil, nil, nil)
	body := `{"context":{"prefix":"const user =","suffix":"","language":"typescript","filePath":"test.ts","cursorOffset":12}}`
	response := httptest.NewRecorder()
...
func TestAuthenticatedCompletionAndRegistration(t *testing.T) {
	...
	router := newRouter(logger, completion.NewService(providers.MockProvider{}), service, true, false, nil, nil, nil)
...
func TestStreamingCompletionEndpoint(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	router := newRouter(logger, completion.NewService(providers.MockProvider{}), auth.New("", 1, 0), false, true, nil, nil, nil)
	body := `{"context":{"prefix":"const user =","suffix":"","language":"typescript","filePath":"test.ts","cursorOffset":12}}`
	response := httptest.NewRecorder()
